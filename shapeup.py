...
This will make it less similar to can-top (missed detections) or bottle-top glare (double detections).
...
class Norm:
    ...
    def __repr__(self):
        return self.__class__.__name__ + repr((self.template, self.objects, self.normal, self.small))

    def __init__(self, template='bottle-top_has_bottle_and_also_its_box_slot.xml', objects=['can-top', 'bottle-top'],
        ...
        matches = (o for o in ET.parse(template).getroot().iter('object') if o.findtext('name') in objects)
        bndboxes = [o.find('bndbox') for o in matches]
        if not bndboxes:
            self.normal = self.small = None
            return

        dX = [int(bndbox.findtext('xmax')) - int(bndbox.findtext('xmin')) for bndbox in bndboxes]
        dY = [int(bndbox.findtext('ymax')) - int(bndbox.findtext('ymin')) for bndbox in bndboxes]

        self.normal = (median(dX), median(dY))
        self.small = (min(dX), min(dY))

    def __call__(self, sloppy, names=['bottle-top'], difficult=True, copy=False, normalized_dir='normalized'):
...
if __name__ == '__main__':
    ...
    argparser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                        ...
                                        epilog="""Examples:
$ rm -rf normalized
$ python3 shapeup.py *bottle*.xml  # Use together with the default template
bottle-top_has_bottle_and_also_its_box_slot.xml
single_bottle-top_capsule_in_single_box.xml
Shapeup bottle-top size (21, 33) to (37.0, 35.5)

$ ls normalized/  # If files were up-shaped, they will appear in this subfolder next to the original.
single_bottle-top_capsule_in_single_box.xml

$ python3 shapeup.py -c *bottle*.xml > normalized/log.txt # Copy/modify all files, documenting what was done to them.
$ ls normalized/
bottle-top_has_bottle_and_also_its_box_slot.xml
log.txt
single_bottle-top_capsule_in_single_box.xml
""")
    ...
    argparser.add_argument('-t', '--template', action='store',
                           default=os.path.join(makemake.prog_dir, 'bottle-top_has_bottle_and_also_its_box_slot.xml'),
                           help="Template pascal voc xml file for defining normal and smallest bndbox sizes (bottle-top_has_bottle_and_also_its_box_slot.xml)")
    argparser.add_argument('-to', '--template_object', action='store', default=['can-top', 'bottle-top'],
                           nargs='+', help="Template object(s) for defining normal and smallest bndbox sizes (can-top bottle-top)")
    argparser.add_argument('-o', '--object', action='store', default=['bottle-top'],
                           nargs='+', help="Object(s) to shape-up (bottle-top)")
    argparser.add_argument('-d', '--difficult', action='store', default=1,
                           type=int, help="Mark shaped-up objects difficult (1)")
    argparser.add_argument('-c', '--copy', action='store_true', help="Create a copy even if not modified")
    argparser.add_argument('-n', '--normalized_dir', action='store', default='normalized',
                           help="Target directory for normalized files")
